...
class CloudDistribution():
    def __init__(self, dir = None):
        ...
        gaussian_fit_options = {'flucCor_switch': False, 'linear_bias_switch': True, 'debug_flag': False, 'offset_switch': True}
    ...
    def expdecay(self,t,A,tau):
        return A*np.exp(-1*t / tau)
    
    def fitexpdecay(self,nums, times):
        max_num = max(nums)
        p_0 = [max_num,1] #fit guess       
        coef, outmat = curve_fit(self.expdecay,times,nums,p0 = p_0)
        return coef
    
    def lifetime(self, timevar):
        if self.does_var_exist(timevar):
            coefs = self.fitexpdecay(self.dists['getAtomNumber'], self.dists[timevar])
            print("Lifetime: %2.2f sec"%coefs[1])
            return coefs[1]
        else:
            print(timevar+' does not exist!')
...
if __name__ == "__main__":
    dir = r"D:\ACMData\Optimization\MoveDipoleLifetime\2014-01-17\\"
    ...
    my_dists.lifetime('MoveHoldTime')
