...
def get_intensity_noise(img, num_bins=50, threshold = 0):
    mean_img = image_subtract(0.5 * (img.atom_image_trunc + img.light_image_trunc), img.dark_image_trunc)
    diff = image_subtract(img.light_image_trunc, img.atom_image_trunc)
...
if __name__ == "__main__":
    ...
    for ii, fname in enumerate(dist.filelist):
        ...
        this_sat = this_bins / sat_counts
        plt.plot(this_sat, this_vars, '.', color='b')
        most_counts = max((most_counts, max(this_bins)))
    plt.plot([0, float(most_counts)/sat_counts], [0, 2.0*most_counts], color='red', linewidth=3, label='Shot Noise Limited')
    plt.xlim(0, 2)
    plt.ylim(0, 30000)
